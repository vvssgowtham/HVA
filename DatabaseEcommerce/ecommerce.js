...
const ECommerce = async () => {
  try {
    ...
    console.log("Ecommerce DB created successfully..");

    const collection = db.collection("catalog");
    console.log("Collection Created Successfully...");

    // Example categories data structure (replace with your actual data)
    ...
    // const created = await collection.insertOne(categoriesData);
    // console.log(created);
    ...
    const newProduct = {
      ...
      vendor: "Myntra",
      badge_text: "Wedding Special",
      image: "https://example.com/messi1.jpg",
      second_image: "https://example.com/messi2.jpg",
    };

    // 1. CREATE - Insert a new product into an existing category
    //UpdateOne contains (filter,modificationFunction,optional)
    // const insertData = await collection.updateOne(
    //   { "categories.category_name": "Kids" }, // This is the correct filter
    //   {
    //     $push: { "categories.$.category_products": newProduct },
    //   },
    //   { upsert: true }
    // );
    ...
    // Without projection: MongoDB returns the entire document if the filter { "categories.category_name": "Kids" } matches, meaning you'll get the whole categories array, including "Men" and "Women."
    // With the categories.$ projection: The $ positional operator tells MongoDB to return only the specific element (in this case, the "Kids" category) that matches the query condition from the categories array.

    //If projection is not given and only categories is given with category_name then
    const readData = await collection.findOne(
      { "categories.category_name": "Kids" }, // This is the filter to find the category "Kids"
      //Projection specifies which fields to include or exclude in the returned document. By default, all fields are included.
      { projection: { "categories.$": 1 } } // This limits the result to only the "Kids" category
    );
    console.log(
      "Products in Kids category:",
      readData.categories[0].category_products
    );

    // 3. Update: Write a script to update the price of a specific product.
    const updatedData = await collection.updateOne(
      { "categories.category_name": "Kids", "categories.category_products.id": "1k" },
      { $set: { "categories.$.category_products.$[product].price": 1300 } },
      { arrayFilters: [{ "product.id": "1k" }], upsert: true }
    );
